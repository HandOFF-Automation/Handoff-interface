import type { FundsTransferFormState } from './funds-types'

type TransferModalContentProps = {
  form: FundsTransferFormState
  onChange: (next: FundsTransferFormState) => void
}

export default function TransferModalContent({ form, onChange }: TransferModalContentProps) {
  return (
    <>
      <FundsField label="Asset" description="Choose which asset you want to transfer.">
      ...
      <FundsField label="Amount" description="Enter how much you want to transfer.">
      ...
      <FundsField label="Destination" description="Enter the destination account or wallet address.">
        <input value={form.destination} onChange={(event) => onChange({ ...form, destination: event.target.value })} placeholder="0xA55C...113F" style={inputStyle} />
      </FundsField>

      <FundsSummary fromLabel="Source Account" fromValue="Primary Investing Vault" toLabel="Destination" toValue={form.destination.trim() || 'Not set yet'} />
    </>
  )
}
