...
export default function CanvasPortfolioConditionSidebar({ active, node, onClose, onMetricChange, onComparatorChange, onValueChange }: { active: boolean; node: CanvasNodeRecord | null; onClose: () => void; onMetricChange: (value: CanvasPortfolioMetric) => void; onComparatorChange: (value: CanvasIfComparator) => void; onValueChange: (value: string) => void }) {
  ...
  return <aside aria-hidden={!active} style={{ position: 'absolute', top: 72, left: 18, width: 280, minHeight: 188, maxHeight: 'calc(100vh - 164px)', borderRadius: 20, border: '1px solid var(--canvas-panel-divider)', background: 'color-mix(in srgb, var(--canvas-surface) 92%, transparent)', boxShadow: '0 16px 40px var(--canvas-shadow-soft)', backdropFilter: 'blur(16px)', display: 'flex', flexDirection: 'column', overflow: 'hidden', pointerEvents: active ? 'auto' : 'none', opacity: active ? 1 : 0, transform: active ? 'translateX(0)' : 'translateX(-16px)', transition: 'opacity 180ms ease, transform 180ms ease', zIndex: 3 }}>
    <CanvasNodeSidebarHeader title="Portfolio Condition" description="Reads portfolio state instead of market-only signals." helpTitle="Portfolio Condition" helpBody="Use this node when the strategy should branch using portfolio context such as cash, exposure, open positions, or drawdown." closeLabel="Close portfolio condition sidebar" onClose={onClose} />
    <div ref={containerRef} style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 18, display: 'flex', flexDirection: 'column', gap: 10 }}>
      <CanvasSidebarFieldSection title="Portfolio Metric" description="Choose which portfolio metric this node should evaluate.">
        <button type="button" onClick={() => setIsMetricOpen((current) => !current)} style={{ width: '100%', minHeight: 54, borderRadius: 16, border: '1px solid var(--canvas-panel-divider)', background: 'var(--canvas-surface-soft)', padding: '0 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, cursor: 'pointer' }}><span style={{ display: 'flex', alignItems: 'center', gap: 12 }}><span style={{ width: 28, height: 28, borderRadius: '999px', border: '1px solid var(--canvas-panel-divider)', background: 'var(--canvas-dashboard-card-bg)', color: 'var(--canvas-accent)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flex: 'none' }}><Wallet size={16} weight="fill" /></span><span style={{ color: 'var(--canvas-text-primary)', fontFamily: 'var(--canvas-font-sans)', fontSize: 13, fontWeight: 700 }}>{selectedMetric.label}</span></span><CaretDown size={14} weight="bold" style={{ color: 'var(--canvas-text-secondary)' }} /></button>
        {isMetricOpen ? <DropdownMenu open={isMetricOpen} boundaryRef={containerRef} groups={metricGroups} position="bottom" portalToBody onItemClick={(item) => { onMetricChange(item.value as CanvasPortfolioMetric); setIsMetricOpen(false) }} /> : null}
      </CanvasSidebarFieldSection>
      <CanvasSidebarFieldSection title="Comparator" description="Choose how this metric should be compared.">
        <button type="button" onClick={() => setIsComparatorOpen((current) => !current)} style={{ width: '100%', minHeight: 54, borderRadius: 16, border: '1px solid var(--canvas-panel-divider)', background: 'var(--canvas-surface-soft)', padding: '0 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, cursor: 'pointer' }}><span style={{ color: 'var(--canvas-text-primary)', fontFamily: 'var(--canvas-font-sans)', fontSize: 13, fontWeight: 700 }}>{selectedComparator.label}</span><CaretDown size={14} weight="bold" style={{ color: 'var(--canvas-text-secondary)' }} /></button>
        {isComparatorOpen ? <DropdownMenu open={isComparatorOpen} boundaryRef={containerRef} groups={comparatorGroups} position="bottom" portalToBody onItemClick={(item) => { onComparatorChange(item.value as CanvasIfComparator); setIsComparatorOpen(false) }} /> : null}
      </CanvasSidebarFieldSection>
      <CanvasSidebarFieldSection title="Value" description="Enter the target value for this portfolio rule." showDivider={false}>
